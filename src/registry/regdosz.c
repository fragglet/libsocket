...
/* RD: Need this for use of atexit() */
#include <stdlib.h>
...
void
regdos_cleanup2(void)
{
    while ( regdos_cleanup() > 0 ) {;}
}
/* RD: End */
...
regdos_startup(void)
{
    ...
    if (!been_here_before) atexit(regdos_cleanup2);
...
#else /* Platforms other than PF_WIN32 */

/* PF_MSDOS_32_REAL - A special define to make things easier */
#if defined(PF_MSDOS_32) && !defined(AHX_DONT_SWITCH_TO_REAL)
#   define PF_MSDOS_32_REAL
#endif

#include <dosx.h>
#include <regdos.h>
#include <string.h>
#ifndef PF_MSDOS_32_REAL
#   include <getvxdp.h>
#endif
#include "lstack.h"
...
#if defined(PF_MSDOS_32_REAL)
typedef LPVOID_RM           VWIN32_PROC;
#else
typedef VXD_PROC            VWIN32_PROC;
#endif
...
get_vwin32_proc(void)
{
	...
	return rm;
#else
	return get_vxd_proc(VWIN32_VXD_ID);
#endif /* PF_MSDOS_32_REAL */
}
...
#elif defined(PF_MSDOS) || defined(PF_MSDOS_16) || defined(PF_WIN16)

/* Macros for the 16 bit platforms are easy. They just need to push
 * the function parameters as is, as 32 bit DWORDs. */
...
#   define REGDOS_SET_VAL(val_var, val_var_size) \
	LSTACK_PUSH_DATA(stx, &val_var, sizeof(DWORD));

#   define REGDOS_SET_PTR(val_var, val_var_size) \
	REGDOS_SET_VAL(val_var, sizeof(DWORD));

#   define REGDOS_SET_BUF(r, l, size) \
	REGDOS_SET_VAL(l, sizeof(DWORD));

#   define REGDOS_SET_VAR(ptr, var, var_size) \
	REGDOS_SET_VAL(var, sizeof(DWORD));
...
#else
#   error PF_MSDOS_32 with AHX_DONT_SWITCH_TO_REAL
#endif
...
#if defined(__DJGPP__)  || defined(__BORLANDC__) || defined(__STDC___)
	return 0;   /* :) */
#endif
}
#endif /* TEST */
