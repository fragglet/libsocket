                         ...
                         struct sockaddr *name, size_t *namelen)
{
	...
	_farpokex (SocketP, 0, &params, sizeof (WSOCK_GETPEERNAME_PARAMS));
	_farpokex (SocketP, 3 * 4, name, *namelen);

	__wsock_callvxd (WSOCK_GETPEERNAME_CMD);

	if (_VXDError && _VXDError != 0xffff) return -1;

	_farpeekx (SocketP, 0, &params, sizeof (WSOCK_GETPEERNAME_PARAMS));
