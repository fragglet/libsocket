...
char *mstcp_getkeystr (char *name);
...
char **mstcp_getdnsaddrs (void)
{
    ...
    if (mstcp_dnsenabled() != 1) return(NULL);
    ...
    q = strtok(buffer, ","); mstcp_dnsaddrs[0] = strdup(q);
...
char *mstcp_gethostname (void) { return(mstcp_getkeystr("HostName")); }
...
char *mstcp_getdomainname (void) { return(mstcp_getkeystr("Domain")); }
