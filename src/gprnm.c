...
int getpeername (int s, struct sockaddr *name, size_t *namelen)
{
    ...
    /* Not connected? Only checking this for connection oriented sockets. */
    if ((lsd->type == SOCK_STREAM) && !lsd->connected) {
	errno = ENOTCONN;
	return (-1);
    }

    /* Check the address params */
    ...
    if (lsd->interface->getpeername == NULL) {
	errno = EOPNOTSUPP;
	return (-1);
    }
    ret = lsd->interface->getpeername (lsd, name, namelen);
    return (ret);
}
